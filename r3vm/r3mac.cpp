...
#if defined(LINUX) || defined(RPI) || defined(MAC)
#include <unistd.h>
#include <dirent.h>
#include <sys/mman.h>
#include <sys/types.h>
...
{
  if (strnicmp(str,"|MAC|",5)==0) {	// MAC=LIN??
    return str+5;
  }
  return nextcr(str);
}
...
memd=srcw*srch*4; // direccion de inicio MAC
memdsize+=(srcw*srch)<<2;
memcode=(int*)malloc(sizeof(int)*memcsize);
memdata=(char*)malloc(memdsize); // MAC need the XRES YRES prior this
...
//	case ADR:NOS++;*NOS=TOS;TOS=(int64_t)&memdata[op>>8];continue;		// LIT adr
	case ADR:NOS++;*NOS=TOS;TOS=op>>8;continue;		// LiT adr MAC
	...
	case FECH:TOS=*(int*)&memdata[TOS];continue;//@
	case CFECH:TOS=*(char*)&memdata[TOS];continue;//C@
	case QFECH:TOS=*(int64_t*)&memdata[TOS];continue;//Q@	
	case FECHPLUS:NOS++;*NOS=TOS+4;TOS=*(int*)&memdata[TOS];continue;//@+
	case CFECHPLUS:NOS++;*NOS=TOS+1;TOS=*(char*)&memdata[TOS];continue;// C@+
	case QFECHPLUS:NOS++;*NOS=TOS+8;TOS=*(int64_t*)&memdata[TOS];continue;//Q@+		
	case STOR:*(int*)&memdata[TOS]=(int)*NOS;NOS--;TOS=*NOS;NOS--;continue;// !
	case CSTOR:*(char*)&memdata[TOS]=(char)*NOS;NOS--;TOS=*NOS;NOS--;continue;//C!
	case QSTOR:*(int64_t*)&memdata[TOS]=*NOS;NOS--;TOS=*NOS;NOS--;continue;//Q!
	case STOREPLUS:*(int*)&memdata[TOS]=*NOS;NOS--;TOS+=4;continue;// !+
	case CSTOREPLUS:*(char*)&memdata[TOS]=*NOS;NOS--;TOS++;continue;//C!+
	case QSTOREPLUS:*(int64_t*)&memdata[TOS]=*NOS;NOS--;TOS+=8;continue;//Q!+
	case INCSTOR:*(int*)&memdata[TOS]+=*NOS;NOS--;TOS=*NOS;NOS--;continue;//+!
	case CINCSTOR:*(char*)&memdata[TOS]+=*NOS;NOS--;TOS=*NOS;NOS--;continue;//C+!
	case QINCSTOR:*(int64_t*)&memdata[TOS]+=*NOS;NOS--;TOS=*NOS;NOS--;continue;//Q+!
	case TOA:REGA=TOS;TOS=*NOS;NOS--;continue; //>A
	case ATO:NOS++;*NOS=TOS;TOS=REGA;continue; //A> 
	case AF:NOS++;*NOS=TOS;TOS=*(int*)&memdata[REGA];continue;//A@
	case AS:*(int*)&memdata[REGA]=TOS;TOS=*NOS;NOS--;continue;//A! 
	case AA:REGA+=TOS;TOS=*NOS;NOS--;continue;//A+ 
	case AFA:NOS++;*NOS=TOS;TOS=*(int*)&memdata[REGA];REGA+=4;continue;//A@+ 
	case ASA:*(int*)&memdata[REGA]=TOS;TOS=*NOS;NOS--;REGA+=4;continue;//A!+
	case TOB:REGB=TOS;TOS=*NOS;NOS--;continue; //>B
	case BTO:NOS++;*NOS=TOS;TOS=REGB;continue; //B> 
	case BF:NOS++;*NOS=TOS;TOS=*(int*)&memdata[REGB];continue;//B@
	case BS:*(int*)&memdata[REGB]=TOS;TOS=*NOS;NOS--;continue;//B! 
	case BA:REGB+=TOS;TOS=*NOS;NOS--;continue;//B+ 
	case BFA:NOS++;*NOS=TOS;TOS=*(int*)&memdata[REGB];REGB+=4;continue;//B@+ 
	case BSA:*(int*)&memdata[REGB]=TOS;TOS=*NOS;NOS--;REGB+=4;continue;//B!+
	case MOVED://MOVE 
//		W=(int64_t)*(NOS-1);op=(int64_t)*NOS;
//		while (TOS--) { *(int*)W=*(int*)op;W+=4;op+=4; }
		memcpy((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS<<2);
		NOS-=2;TOS=*NOS;NOS--;continue;
	case MOVEA://MOVE> 
//		W=(int64_t)*(NOS-1)+(TOS<<2);op=(int64_t)(*NOS)+(TOS<<2);
//		while (TOS--) { W-=4;op-=4;*(int*)W=*(int*)op; }
		memmove((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS<<2);
		NOS-=2;TOS=*NOS;NOS--;continue;
	case FILL://FILL
		W=(int64_t)&memdata[*(NOS-1)];op=*NOS;
		while (TOS--) { *(int*)W=op;W+=4; }
		NOS-=2;TOS=*NOS;NOS--;continue;
	case CMOVED://CMOVE 
//		W=(int64_t)*(NOS-1);op=(int64_t)*NOS;
//		while (TOS--) { *(char*)W=*(char*)op;W++;op++; }
		memcpy((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS);
		NOS-=2;TOS=*NOS;NOS--;continue;
	case CMOVEA://CMOVE> 
//		W=(int64_t)*(NOS-1)+TOS;op=(int64_t)*NOS+TOS;
//		while (TOS--) { W--;op--;*(char*)W=*(char*)op; }
		memmove((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS);
		NOS-=2;TOS=*NOS;NOS--;continue;
	case CFILL://CFILL
		W=(int64_t)&memdata[*(NOS-1)];op=*NOS;
...
//		while (TOS--) { *(uint64_t*)W=*(uint64_t*)op;W+=8;op+=8; }
		memcpy((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS<<3);	
		NOS-=2;TOS=*NOS;NOS--;continue;
	case QMOVEA://MOVE> 
//		W=(uint64_t)*(NOS-1)+(TOS<<3);op=(uint64_t)*NOS+(TOS<<3);
//		while (TOS--) { W-=8;op-=8;*(uint64_t*)W=*(uint64_t*)op; }
		memmove((void*)&memdata[*(NOS-1)],(void*)&memdata[*NOS],TOS<<3);		
		NOS-=2;TOS=*NOS;NOS--;continue;
	case QFILL://QFILL
		W=(uint64_t)&memdata[*(NOS-1)];op=*NOS;
	...
	case LOAD: //LOAD: // 'from "filename" -- 'to
        ...
        file=fopen((char*) &memdata[TOS],"rb");
        TOS = *NOS ;
	NOS--;
        if (file==NULL) continue;
        do { W=fread((void*)&memdata[TOS],sizeof(char),1024,file); TOS+=W; } while (W==1024);
    ...
    case SAVE: //SAVE: // 'from cnt "filename" --
        if (TOS==0||*NOS==0) { 
#if defined(MAC) 
 			remove((char*) &memdata[TOS]);
#else
			DeleteFile((char*)TOS);
#endif			
		NOS-=2;TOS=*NOS;NOS--;continue; }
        file=fopen( (char*) &memdata[TOS],"wb") ;
        TOS=*NOS;NOS--;
        if (file==NULL) { NOS--;TOS=*NOS;NOS--;continue; }
        fwrite((void*)&memdata[*NOS],sizeof(char),TOS,file);
        fclose(file);
	...
	closedir(dirp);
      ...
      dirp=opendir((char*) &memdata[TOS]) ;
      
      if (dirp!=NULL) {
	dp=readdir(dirp);
	if(dp)
	  TOS=((int64_t)dp) - ((int64_t) memdata) ;
	else
	  TOS = 0 ;
      }
      else {
	...
	case INKA:	//'INK
		NOS++;*NOS=TOS;TOS=((int64_t)&gr_color1)-(int64_t)memdata;continue;
	case ALPHA:	// ALPHA
...
gr_init(filename,srcw,srch,scrf,(Uint32 *)memdata);
SDL_StartTextInput();
/*
while (key!=27) {
	gr_cls(0xff00);
	gr_color1=0xff0000;
	gr_line(10,10,400,200);
	gr_redraw();
	r3update();
